import { Button, Col, Form, Input, Row, Select } from "antd";
import { useEffect, useState } from "react";
import { getListCity } from "../../services/cityService";
import "./searchform.scss";
import { SearchOutlined } from "@ant-design/icons";
import { IoLocationOutline } from "react-icons/io5";

function SearchForm(props) {
  const { onSearch } = props;
  const [city, setCity] = useState([]);
  useEffect(() => {
    const fetchAPI = async () => {
      const response = await getListCity();
      // console.log(response);
      if (response) {
        const objAll = {
          key: 0,
          value: "All",
        };
        setCity([objAll, ...response]);
      }
    };
    fetchAPI();
  }, []);

  // console.log(city);

  const handleFinish = (values) => {
    let cityValue = values.city || "";
    cityValue = values.city === "All" ? "" : cityValue;
    const payload = { city: cityValue, keyword: values.keyword || "" };
    if (typeof onSearch === "function") {
      onSearch(payload);
    }
  };
  return (
    <>
      <div className="search-form">
        <h1 className="title">
          Tìm việc làm nhanh 24h, việc làm mới nhất trên toàn quốc.
        </h1>
        <div className="sub-title">
          Tiếp cận 40,000+ tin tuyển dụng việc làm mỗi ngày từ hàng nghìn doanh
          nghiệp uy tín tại Việt Nam
        </div>

        {city && (
          <Form onFinish={handleFinish}>
            <Row
              gutter={{ xs: 12, sm: 16, md: 20, lg: 24 }}
              justify="center"
              align="middle"
            >
              <Col xs={24} md={8} lg={6} xxl={6}>
                <Form.Item name="city">
                  <Select
                    style={{ height: "50px" }}
                    options={city}
                    placeholder={
                      <>
                        <IoLocationOutline /> Địa điểm
                      </>
                    }
                  />
                </Form.Item>
              </Col>
              <Col xs={24} md={10} lg={8} xxl={6}>
                <Form.Item name="keyword">
                  <Input
                    style={{ height: "50px" }}
                    placeholder="Vị trí tuyển dụng"
                  />
                </Form.Item>
              </Col>
              <Col xs={24} sm={12} md={6} lg={4} xxl={3}>
                <Form.Item>
                  <Button
                    block
                    type="primary"
                    style={{ backgroundColor: "#00b14f", height: "50px" }}
                    htmlType="submit"
                  >
                    <SearchOutlined /> Tìm kiếm
                  </Button>
                </Form.Item>
              </Col>
            </Row>
          </Form>
        )}
      </div>
    </>
  );
}
export default SearchForm;
